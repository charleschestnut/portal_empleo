...
def profile_delete(request):
    def all_rated_by_clients_ratings(user_id):
        c_ratings = ClientRating.objects.filter(rater_person__user_id=int(user_id)).values_list('rated_person__user_id', flat=True)
        return c_ratings

    def all_rated_by_worker_ratings(user_id):
        w_ratings = WorkerRating.objects.filter(rater_person__user_id=int(user_id)).values_list('rated_person__user_id', flat=True)
        return w_ratings
    ...
    for new_avg_user in users_cr:
        update.update_avg_rating(new_avg_user, 'CLIENT', user_to_delete.id)

    for new_avg_user1 in users_wr:
        update.update_avg_rating(new_avg_user1, 'WORKER', user_to_delete.id)
